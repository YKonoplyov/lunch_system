...
class GetVoteResultsView(APIView):
    ...
    def get_vote_results(self, filter_date: timezone = timezone.now().date()):
        ...
        vote_results = (
            queryset.annotate(
                votes_count=Count(
                    "votes", filter=Q(
                        votes__date_voted=filter_date)
                )
            )
            .annotate(menu_name=F("name"))
            .values("menu_name")
            .annotate(votes_count=Coalesce("votes_count", 0))
    ...
    def get(self, request):
        ...
        if date_voted:
            filtering_date = date_voted

        results_data = self.get_vote_results(filtering_date)

        if not results_data:
            return Response(
                {"message": "There is no vote " f"data for {date_voted}"}
            )

        serializer_class = self.get_serializer_class(request=request)
        serializer = serializer_class(data=results_data, many=True)
